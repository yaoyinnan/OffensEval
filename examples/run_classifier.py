...
from src.transformers import my_compute_metrics as compute_metrics, plot_confusion_matrix
...
def test(args, model, tokenizer, prefix=""):
    ...
    for test_task, test_output_dir in zip(test_task_names, test_outputs_dirs):
        ...
        # plot_confusion_matrix(result['confusion_matrix'], processor.get_labels(), "Confusion Matrix")
        ...
        with open(output_test_file, "w") as writer:
            ...
            writer.write("%s = %s\n" % ("loss", str(test_loss)))
        writer.close()
        # plt.savefig(os.path.join(test_output_dir, args.task_name + '_cm.png'), format='png')
