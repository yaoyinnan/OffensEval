...
class OffensEval2020Task1EnglishProcessor(DataProcessor):
    ...
    def get_test_examples(self, data_dir):
        """See base class."""
        return self._create_examples(
            self._read_tsv(os.path.join(data_dir, "test-goldlabels-a.tsv")), "test")
...
class OffensEval2020Task2EnglishProcessor(DataProcessor):
    """Processor for the OffensEval2020Task1English data set (My version)."""
    ...
    def get_train_examples(self, data_dir):
        """See base class."""
        logger.info("LOOKING AT {}".format(os.path.join(data_dir, "55train.tsv")))
        return self._create_examples(
            self._read_tsv(os.path.join(data_dir, "55train.tsv")), "train")
    ...
    def get_test_examples(self, data_dir):
        """See base class."""
        return self._create_examples(
            self._read_tsv(os.path.join(data_dir, "test-goldlabels-b.tsv")), "test")
    ...
    def _create_examples(self, lines, set_type):
        ...
        for (i, line) in enumerate(lines):
            if i == 0:
                continue
            text_b = "Does Posts containing contain the target audience of profanity?"
            if set_type in ["train", "dev"]:
                guid = "%s-%s" % (set_type, i)
                label = line[0]
                text_a = line[1]
...
class OffensEval2020Task3EnglishProcessor(DataProcessor):
    """Processor for the OffensEval2020Task1English data set (My version)."""
    ...
    def get_train_examples(self, data_dir):
        """See base class."""
        logger.info("LOOKING AT {}".format(os.path.join(data_dir, "train_e.tsv")))
        return self._create_examples(
            self._read_tsv(os.path.join(data_dir, "train_e.tsv"), '"'), "train")
    ...
    def get_test_examples(self, data_dir):
        """See base class."""
        return self._create_examples(
            self._read_tsv(os.path.join(data_dir, "test-goldlabels-c.tsv"), '"'), "test")
    ...
    def _create_examples(self, lines, set_type):
        ...
        for (i, line) in enumerate(lines):
            ...
            if set_type in ["train", "dev"]:
                ...
                if sum(num) == 11876 * 3:
                    break
                num[self.get_labels().index(label)] += 1
                text_a = line[1]
                text_a = self.preprocess(text_a)
                examples.append(InputExample(guid=guid, text_a=text_a, text_b=None, label=label))
            elif set_type in ["test"]:
...
my_tasks_num_labels = {
    ...
    "offenseval2020task2english": 2,
    "offenseval2020task3english": 3,
...
my_processors = {
    ...
    "offenseval2020task2english": OffensEval2020Task2EnglishProcessor,
    "offenseval2020task3english": OffensEval2020Task3EnglishProcessor,
...
my_output_modes = {
    ...
    "offenseval2020task2english": "classification",
    "offenseval2020task3english": "classification",
